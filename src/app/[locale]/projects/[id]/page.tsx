...
// Define interfaces for page props and tab content
...
const ProjectPage: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const photoInterval = setInterval(() => {
            if (activeTab === 'photos') {
                setCurrentPhotoSlide((prev) => (prev === Math.ceil(photoGalleryImages.length / 3) - 1 ? 0 : prev + 1));
            }
        }, 4000);
        ...
        const videoInterval = setInterval(() => {
            if (activeTab === 'videos') {
                setCurrentVideoSlide((prev) => (prev === videoGalleryImages.length - 1 ? 0 : prev + 1));
            }
        }, 6000);
    ...
    }, [currentSlide, isTransitioning, backgroundImages.length, activeTab, photoGalleryImages.length, videoGalleryImages.length]);
    ...
    const tabs: TabContent[] = [
                    ...
                    <div className="flex flex-row md:flex-col bg-[#FFFFFF] gap-2 py-3 rounded-full px-7 overflow-x-auto md:overflow-visible   ">
                        {featureTabs.map((item) => (
                            <button
                                ...
                                className={`h-12 w-12 md:h-20 md:w-20 rounded-full flex items-center justify-center flex-shrink-0 transition-colors duration-300 ${activeFeatureTab === item.id ? "bg-black" : "bg-gray-500"
                                    }`}
                            >
                                <span className="text-white text-xs md:text-sm  text-center px-1">{item.name}</span>
                            </button>
                        ))}
                    </div>
                    ...
                    <div className="w-full md:flex-1 border-t md:border-t-0  md:pt-0 pt-13 mt-10  ">
                        ...
                        {activeFeatureTab === 'Details' && (
                            <>{[
                                ...
                                { label: "Windows Type", value: "Double Glazed Soundproof Glass" }
                            ].map((item) => (
                                <div key={item.label} className="border-b border-[#B1B3B4] px-10">
                                    <div className="flex justify-between items-center py-5">
                                        <div className="text-gray-800 capitalize text-sm">{item.label}</div>
                                        <div className="text-gray-700 text-sm">{item.value}</div>
                                    </div>
                                </div>
                            ))}</>
                        ...
                        {activeFeatureTab === 'Amenities' && (
                            <div className="grid grid-cols-2">
                                ...
                                <div className="py-4 px-6 border-b border-[#E5E7EB]">
                                    <div className="text-gray-800 text-sm">24/7 Security Service</div>
                                </div>

                                <div className="py-4 px-6 border-b border-r border-[#E5E7EB]">
                                    <div className="text-gray-800 text-sm">Reception / Luxury Lobby</div>
                                </div>
                                <div className="py-4 px-6 border-b border-[#E5E7EB]">
                                ...
                                <div className="py-4 px-6 border-b border-r border-[#E5E7EB]">
                                    <div className="text-gray-800 text-sm">High-Speed Internet</div>
                                </div>
                                <div className="py-4 px-6 border-b border-[#E5E7EB]">
                                    <div className="text-gray-800 text-sm">Reception / Luxury Lobby</div>
                                </div>
        ...
        },
        {
            id: 'photos',
            name: 'photos',
            icon: <svg width="30" height="15" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M28.5 21V27C28.5 27.8284 27.8284 28.5 27 28.5H3C2.17158 28.5 1.5 27.8284 1.5 27V24M28.5 21V3C28.5 2.17158 27.8284 1.5 27 1.5H3C2.17158 1.5 1.5 2.17158 1.5 3V24M28.5 21L20.2243 15.4829C19.7764 15.1842 19.2028 15.1485 18.7213 15.3892L1.5 24" stroke="#060B0E" stroke-width="1.5" stroke-linejoin="round" />
            </svg>
            ,
            content: (
                <div className="relative overflow-hidden py-3">
                    ...
                    <div className="relative">
                        ...
                        >
                            {Array.from({ length: Math.ceil(photoGalleryImages.length / 3) }).map((_, groupIndex) => (
                                <div key={groupIndex} className="w-full flex-shrink-0">
                                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-4 lg:gap-6">
                                        {photoGalleryImages.slice(groupIndex * 3, (groupIndex + 1) * 3).map((_, imgIndex) => {
                                            const actualIndex = groupIndex * 3 + imgIndex;
                                            return (
                                                <div key={actualIndex} className="aspect-video bg-gray-200 rounded-md overflow-hidden relative">
                        ...
                        {/* Arrow navigation */}
                        <button
                            onClick={() => setCurrentPhotoSlide(prev => (prev === 0 ? Math.ceil(photoGalleryImages.length / 3) - 1 : prev - 1))}
                        ...
                        <button
                            onClick={() => setCurrentPhotoSlide(prev => (prev === Math.ceil(photoGalleryImages.length / 3) - 1 ? 0 : prev + 1))}
                    ...
                    <div className="flex justify-center gap-1 mt-3">
                        {Array.from({ length: Math.ceil(photoGalleryImages.length / 3) }).map((_, index) => (
                            <button
                                key={index}
                                onClick={() => setCurrentPhotoSlide(index)}
                                className={`w-2 h-2 rounded-full transition-colors ${currentPhotoSlide === index ? 'bg-blue-600' : 'bg-gray-300'}`}
        ...
        },
        {
            ...
            name: 'videos',
            icon: <svg width="30" height="15" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M3 6H0V27C0 28.65 1.35 30 3 30H24V27H3V6ZM27 0H9C7.35 0 6 1.35 6 3V21C6 22.65 7.35 24 9 24H27C28.65 24 30 22.65 30 21V3C30 1.35 28.65 0 27 0ZM27 21H9V3H27V21ZM15 5.25V18.75L24 12L15 5.25Z" fill="#060B0E" />
            </svg>
            ,
            content: (
                <div className="relative overflow-hidden space-y-3 py-3">
                    ...
                    <div className="relative">
                        ...
                        {/* Arrow navigation */}
                        <button
                            onClick={() => setCurrentVideoSlide(prev => (prev === 0 ? videoGalleryImages.length - 1 : prev - 1))}
                            className="absolute left-1 top-1/2 -translate-y-1/2 w-8 h-8 bg-black/50 rounded-full flex items-center justify-center text-white z-10 hover:bg-black/70"
                        ...
                        <button
                            onClick={() => setCurrentVideoSlide(prev => (prev === videoGalleryImages.length - 1 ? 0 : prev + 1))}
                            className="absolute right-1 top-1/2 -translate-y-1/2 w-8 h-8 bg-black/50 rounded-full flex items-center justify-center text-white z-10 hover:bg-black/70"
                    ...
                    <div className="flex justify-center gap-1 mt-2">
                        {videoGalleryImages.map((_, index) => (
                            <button
                                ...
                                className={`w-2 h-2 rounded-full transition-colors ${currentVideoSlide === index ? 'bg-blue-600' : 'bg-gray-300'}`}
            ...
            >
                ...
                >
                    {backgroundImages.map((image, index) => (
                        <div key={index} className="relative w-full h-[90vh]">
                            ...
                            {/* Slide content - only show for current slide */}
                            {index === currentSlide && (
                ...
                <div className="absolute left-13 top-1/2 transform -translate-y-1/2 flex flex-col gap-1">
                    {backgroundImages.map((_, i) => (
                        <button
                            ...
                            className={`w-3 h-${i === currentSlide ? '6' : '3'} rounded-full transition-all duration-300 ${i === currentSlide ? 'bg-[#E2A12D]' : 'bg-white'}`}
                ...
                <div className="absolute left-8 bottom-8 z-10">
                    <button
                        ...
                        className={`transition-all duration-300 hover:scale-110 ${currentSlide === backgroundImages.length - 1 ? ' cursor-not-allowed' : ''}`}
            ...
            <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:space-y-1 space-y-[150px] sm:py-8">
                <div className="flex flex-col items-center">
                    ...
                    <div className="w-full text-black text-sm md:text-base px-18  h-40   font-['Lato'] " style={{
                        ...
                        New Administrative Capital - Downtown Plot NO. MU5-39, Area 3275m². (Commercial - Medical - Offices) Consists Of 15 Mixed-Used Floors. Project Consultant Engineer/ Yasser El Beltagy. Location Features Ideal Location in The New Capital KÈNTRO TOWER is Located in The Heart Of Downtown, The Artery Of The New Administrative Capital. NEARBY Minutes Away From The Gold Market and Mosque Of Egypt. Very Close To The Monorail Station, Tourist Towers, and The Green River. Minutes Away From The Masa Hotel and The Iconic Tower.
                    </div>
            ...
            <div className="w-full py-6 md:py-8 bg-gray-100">
                <div className="max-w-5xl mx-auto">
                    <div className="flex flex-col items-center gap-2 mb-4">
                        <h2 className="text-blue-600 text-lg md:text-xl py-6  font-['Lato'] uppercase" style={{
                            fontSize: 'clamp(1rem, 1.6vw, 1.25rem)'
                        }}>
                            Building Features
                        </h2>
                        <div className="text-center text-black  font-['Cinzel'] leading-tight max-w-[300px]" style={{
                    ...
                    <div className="flex flex-wrap justify-center gap-2 mb-6 text-sm">
                        {tabs.map((tab) => (
                            <button
                                ...
                                className={`px-3 py-1.5 rounded-full text-sm flex items-center gap-1 min-w-0 sm:min-w-[120px] md:min-w-[150px] transition-colors duration-300  ${activeTab === tab.id ? "bg-black text-white" : "bg-white text-black"
                                    }`}
                            >
